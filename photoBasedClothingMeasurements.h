...
Mat photoBasedClothingMeasurements(Mat inImg, string debugImage, string fileOut, string fileDebug)
{
	...
	for (int row = 0; row < inImg.rows; ++row)
	{
		...
		{
			debug << f1[i1].l << " " << f1[i1].r << endl;
			debug << f1[i1].pL.x << " " << f1[i1].pL.y << " " << f1[i1].pL.gl << " " << f1[i1].pL.gr << endl;
			debug << f1[i1].pR.x << " " << f1[i1].pR.y << " " << f1[i1].pR.gl << " " << f1[i1].pR.gr << endl;
		}

		debug << "l2" << endl;
		for (int i2 = 0; i2 <= len2; ++i2)
		{
			debug << f2[i2].l << " " << f2[i2].r << endl;
			debug << f2[i2].pL.x << " " << f2[i2].pL.y << " " << f2[i2].pL.gl << " " << f2[i2].pL.gr << endl;
			debug << f2[i2].pR.x << " " << f2[i2].pR.y << " " << f2[i2].pR.gl << " " << f2[i2].pR.gr << endl;
		}
		...
		{
			{
				...
				debug << "color " << cc1[i].color << " left " << cc1[i].l << " right " << cc1[i].r << " left_down " << cc1[i].linkLeftDown << " right_down " << cc1[i].LinkRightDown << endl;
				debug << "edge" << endl;
		...
		for (int idLeftDown = 0, idLeftUp, idRightUp, idRightDown; idLeftDown <= len1; ++idLeftDown)
		{
			...
			if (idLeftUp == -1)
			{
				...
				edgeStack.getEdge(cc2[idLeftDown].edgeID)->setEdge(
					currentImageId,
					-1,
					(f1[idLeftDown].r == f1[idLeftDown].l) ? numeric_limits<double>::max() : ((double)f1[idLeftDown].r - f1[idLeftDown].l) / ((double)EPS),
					false,
					f1[idLeftDown].l,
					pRow,
					f1[idLeftDown].r,
				...
				for (int j = idLeftUp + 1, indexCur = cc1[idLeftUp].edgeID; j <= idRightUp && indexCur != -1; ++j)
				{
					if (cc1[j].color == f1[idLeftDown].color)
					{
						...
						edgeStack.getEdge(newEdgeId)->setEdge(
							currentImageId,
							cc1[j].edgeID,
							(cc1[j].l == cc1[fkc].r) ? numeric_limits<double>::max() : ((double)cc1[j].l - cc1[fkc].r) / ((double)EPS),
							false,
							cc1[fkc].r,
							row - 2,
							cc1[j].l,
				...
				for (int j = idLeftDown; j <= idRightDown; ++j)
				{
					...
					if (cc1[idLeftUp].color != f1[j].color)
					{
						...
						edgeStack.getEdge(cc2[j].edgeID)->setEdge(
							currentImageId,
							-1,
							(f1[j].r == f1[j].l) ? numeric_limits<double>::max() : ((double)f1[j].r - f1[j].l) / ((double)EPS),
							false,
							f1[j].l,
							pRow,
							f1[j].r,
